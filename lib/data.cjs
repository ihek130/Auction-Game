...
const playerPool = PLAYER_ROWS.map((row, id) => {
  const [name, code, category, style, era, t20, odi, test] = row.split("|");
  return {
    id, name, code, country:COUNTRIES[code][0], flag:COUNTRIES[code][1],
    category, style, era, ratings:{t20:+t20, odi:+odi, test:+test}
  };
});


module.exports = { playerPool, CATEGORIES, FORMATS };
