...
function next(s, now) {
  ...
  // The wildcard stage draws from every category at random, the same way as
  // any other stage; nobody is asked to pick a category first.
  openLot(s, now);
}
function settle(s) {
...
// The game rating is withheld until the auction is over: players bid on the
// real career numbers, and the rating is revealed with the result. Hiding it
// here rather than in the browser means nobody can peek.
function player(id, format, reveal) {
  const p = playerPool[id];
  return { id: p.id, name: p.name, flag: p.flag, country: p.country, category: p.category,
    role: CATEGORIES[p.category].singular, era: p.era, stats: p.stats?.[format] || null,
    ...(reveal ? { rating: p.ratings[format] } : {}) };
}
function view(s, you, now) {
  const reveal = s.phase === 'finished';
  const seats = s.seats.map((p, i) => p && ({ name: p.name, wallet: p.wallet, bot: !!p.bot,
    maxBid: p.squad.length < 12 ? maxBid(s, i) : 0,
    squad: p.squad.map(x => ({ ...player(x.id, s.format, reveal), price: x.price, stage: x.stage })),
  ...
  return { id: s.id, format: s.format, revision: s.revision, expiresAt: s.expiresAt, serverTime: now, you,
    ...
    current: s.current === null ? null : player(s.current, s.format, reveal), turn: s.turn, deadline: s.deadline,
    high: s.high, leader: s.leader, history: s.history.slice(-12),
    // Which seats are out of this lot (passed, or ineligible when it opened),
    // so the client can explain a turn that never comes.
    passed: s.passed,
    // Which seats are out of this lot (passed, or ineligible when it opened),
    // so the client can explain a turn that never comes.
    passed: s.passed,
