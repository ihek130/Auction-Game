...
test('two independent HTTP clients create, invite, bid, pass and reconnect',async()=>{
  ...
  try {
    for(const url of ['/','/app.js','/styles.css','/favicon.svg'])assert.equal((await fetch(base+url)).status,200);
    const a=await req({type:'create',name:'Hassan',format:'t20'});assert.equal(a.status,200);
    ...
    const state=await req(null,room,a.token);assert.equal(state.state.seats[1].name,'Jaffer');
